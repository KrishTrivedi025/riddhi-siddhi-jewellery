...
async function BanksData() {
    ...
    const bankTotal = accounts.filter(a => !a.isCash).reduce((sum, a) => sum + a.currentBalance, 0)
    const cashTotal = accounts.filter(a => a.isCash).reduce((sum, a) => sum + a.currentBalance, 0)
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-card border border-border rounded-xl p-5 relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-4 opacity-5">
                        <Landmark size={70} className="text-blue-400" />
                    </div>
                ...
                <div className="bg-card border border-border rounded-xl p-5 relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-4 opacity-5">
                        <Layers size={70} className="text-primary" />
                    </div>
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div><Skeleton className="h-8 w-48 bg-muted" /><Skeleton className="h-4 w-72 mt-2 bg-muted" /></div>
                <Skeleton className="h-10 w-32 bg-muted" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[1, 2, 3].map(i => <Skeleton key={i} className="h-24 bg-muted rounded-xl border border-border" />)}
            </div>
            <Skeleton className="h-64 w-full bg-muted rounded-xl border border-border" />
        </div>
    )
}
