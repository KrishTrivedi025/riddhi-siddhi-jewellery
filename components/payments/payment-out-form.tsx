...
export function PaymentOutForm({ suppliers }: PaymentOutFormProps) {
    ...
    const handleSupplierChange = async (id: string) => {
        ...
        try {
            const data = await getSupplierOutstandingInvoices(id)
    ...
    const handleSubmit = async () => {
        ...
        if (Math.abs(currentModesTotal - totalAmount) > 0.01) {
             setError(`Total Payment (₹${totalAmount}) does not match Split Mode Total (₹${currentModesTotal}).`)
             return
        }
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                        <h1 className="text-lg font-bold text-foreground flex items-center gap-2 leading-tight">
                            <CreditCard size={18} className="text-rose-400 flex-shrink-0" />
                            Record Payment Made
                        </h1>
                        <p className="text-xs text-muted-foreground">Log money paid to a supplier</p>
                    </div>
                </div>
                <Button
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-card border border-border rounded-xl p-5 space-y-4">
                        <div className="space-y-2">
                            ...
                            <Select value={partyId} onValueChange={handleSupplierChange}>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-card border border-border rounded-xl overflow-hidden min-h-[400px] flex flex-col">
                        <div className="p-5 flex items-center justify-between border-b border-border bg-muted/50">
                            <div>
                                <h3 className="text-sm font-semibold text-foreground">Outstanding Purchase Bills</h3>
                                <p className="text-xs text-muted-foreground mt-1">Select which bills you are paying for.</p>
                            </div>
                            <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 overflow-x-auto p-4">
                            {fetchingInvoices ? (
                                ...
                                <div className="flex flex-col items-center justify-center p-12 h-full text-muted-foreground">
                                    ...
                                    No pending bills found for this supplier.
                                </div>
                            ) : invoices.length === 0 && !partyId ? (
                                <div className="flex flex-col items-center justify-center p-12 h-full text-muted-foreground">
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow className="border-border hover:bg-transparent">
                                            ...
                                            <TableHead className="text-muted-foreground text-xs text-right">Balance Due</TableHead>
                                            <TableHead className="text-muted-foreground text-xs text-right w-40">Amount Paying</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                                ...
                                                <TableCell className="text-right">
                                                    <Input
                                                        ...
                                                        className={`bg-background border text-right pr-3 h-8 ${
                                                            inv.amountApplied > 0 
                                                            ? 'border-rose-500/50 text-rose-400 font-bold bg-rose-500/5' 
                        ...
                        <div className="p-5 bg-background flex items-center justify-between text-sm">
                             <div className="flex gap-8">
                                  <div>
                                       ...
                                       <span className="text-rose-400 font-semibold">{formatCurrency(totalOutstanding)}</span>
                                  </div>
                             </div>
