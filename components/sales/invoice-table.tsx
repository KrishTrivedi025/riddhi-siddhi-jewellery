        ...
        <div className="space-y-4">
            ...
            {filtered.length === 0 ? (
                <div className="border border-border rounded-xl p-12 text-center flex flex-col items-center gap-3">
                    ...
                    <Button onClick={() => router.push("/dashboard/sales/new")} variant="ghost" className="text-primary hover:bg-primary/10">
                ...
                <>
                    ...
                    <div className="md:hidden space-y-3">
                        {filtered.map((inv) => (
                            <div key={inv.id} onClick={() => { window.location.href = `/dashboard/sales/${inv.id}` }}
                                ...
                                <div className="flex items-start justify-between gap-2 mb-3">
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground mt-0.5">{inv.invoiceNumber} · {format(new Date(inv.invoiceDate), "dd MMM yyyy")}</p>
                                    </div>
                            ...
                            <TableBody>
                                {filtered.map((inv) => (
                                    <TableRow key={inv.id} className="border-border hover:bg-card/50 cursor-pointer"
                                        onClick={() => { window.location.href = `/dashboard/sales/${inv.id}` }}>
