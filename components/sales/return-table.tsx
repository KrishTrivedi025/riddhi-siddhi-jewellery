...
export function ReturnTable({ returns }: ReturnTableProps) {
    const [search, setSearch] = useState("")

    const filtered = returns.filter((r) => {
        if (!search) return true
        return (
            r.creditNoteNumber?.toLowerCase().includes(search.toLowerCase()) ||
            r.party?.name?.toLowerCase().includes(search.toLowerCase()) ||
            r.saleInvoice?.invoiceNumber?.toLowerCase().includes(search.toLowerCase())
        )
    })
                            ...
                            <TableRow className="border-border">
                                <TableCell colSpan={7} className="text-center py-12">
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <p className="text-muted-foreground text-sm">No credit notes found</p>
                                    </div>
                                </TableCell>
                            </TableRow>
                        ) : (
                            filtered.map((r) => (
                                <TableRow key={r.id} className="border-border hover:bg-card/50 transition-colors">
                                    ...
                                    <TableCell className="text-muted-foreground">
                                        {format(new Date(r.creditNoteDate), "dd MMM yyyy")}
                                    </TableCell>
                                    <TableCell>
